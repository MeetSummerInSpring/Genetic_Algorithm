...
for i in trange(calculate_limit):
    ...
    population_chroms = select(population_chroms, population_values,
                               population_size)
    # 交配
    population_chroms = crossover_mating(population_chroms, chrom_length, mating_rate)
    # 变异
...
func_X = np.arange(gene_min, gene_max, 0.1)
func_Y = func(func_X)
...
plt.title(name, fontsize=18, loc="center")
plt.xlabel("iter_times", fontsize=12, loc="center")
plt.ylabel("fitness_value", fontsize=12, loc="center")
